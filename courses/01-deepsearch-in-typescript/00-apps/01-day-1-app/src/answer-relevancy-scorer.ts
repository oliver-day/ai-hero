...
export const checkAnswerRelevancy = async (opts: {
  ...
  const { object: verdictsResult } = await generateObject({
    model: factualityModel,
  ...
  const totalScore = verdictsResult.verdicts.reduce(
    (sum, verdict) => sum + scores[verdict.verdict],
    0,
  );
  const averageScore = totalScore / verdictsResult.verdicts.length;

  return {
    score: averageScore,
    metadata: {
      statements,
      verdicts: verdictsResult.verdicts,
